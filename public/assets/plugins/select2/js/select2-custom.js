...
$(document).ready(function() {
    ...
    // Pour chaque élément select avec l'attribut data-codeproduit
    $('select[data-codeproduit]').each(function() {
        var codeProduit = $(this).data('codeproduit'); // Récupérer le code produit
        var selectId = '#lieunaissance-' + codeProduit; // Construire l'ID

        // Initialisation de Select2 avec possibilité de saisie
        $(selectId).select2({
            theme: "bootstrap-5",
            width: '100%',
            placeholder: 'Sélectionner le lieu',
            allowClear: true,
            tags: true // Permet la saisie d'options personnalisées
        });
    });
});
