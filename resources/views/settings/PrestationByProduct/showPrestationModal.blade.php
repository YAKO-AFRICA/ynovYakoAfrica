    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="showPrestationModalLabel{{ $item->CodeProduit }}">Liste des prestations</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                @if (!empty($item->typePrestations) && $item->typePrestations->isNotEmpty())
                    <ul class="list-group">
                        @foreach ($item->prestations as $prestation)
                            <li class="list-group-item d-flex justify-content-between align-items-center"
                                id="product-{{ $prestation->id }}">
                                <span>
                                    {{ $prestation->libelle }} (impact :
                                    {{ $prestation->impact == 1 ? 'Sortie de portefeuille' : 'pas de sortie de portefeuille' }})
                                </span>
                                <a class="deleteConfirmation ms-3" data-uuid="{{ $prestation->id }}"
                                    data-type="confirmation_redirect" data-placement="top"
                                    data-token="{{ csrf_token() }}"
                                    data-url="{{ route('setting.prestation_product.destroy', $prestation->id) }}"
                                    data-title="Vous êtes sur le point de retirer la prestation {{ $prestation->libelle }} "
                                    data-id="{{ $prestation->id }}" data-param="0"
                                    data-route="{{ route('setting.prestation_product.destroy', $prestation->id) }}">
