...
<div class="page-content">
                            ...
                            <td>
                                <div class="d-flex order-actions">
                                    ...
                                    <a class="deleteConfirmation ms-3" data-uuid="{{$item->id}}"
                                        ...
                                        data-url="{{route('setting.destroy.motifRejetProposition',$item->id)}}"
                                        data-title="Vous êtes sur le point de supprimer {{$item->libelle}} "
                                        data-id="{{$item->id}}" data-param="0"
                                        data-route="{{route('setting.destroy.motifRejetProposition',$item->id)}}"><i
                                            class='bx bxs-trash' style="cursor: pointer"></i>
                                        </a>
                                </div>
                            </td>
                        </tr>
                        @include('settings.motifs.proposition.editModal', ['item' => $item])
                        @empty
    ...
    @include('settings.motifs.proposition.addModal')
