{{-- <form action="" method="post"> --}}
    ...
    <div>
        ...
        <div class="row g-3 mt-3">
            <div class="card col d-none" style="min-height: 60px">
                    ...
                    <form style="display: none;" id="auDecesForm">
                        ...
                        @php
                            $options = [
                                ...
                                'Le conjoint non divorcé, ni séparé de corps' => 'Le conjoint non divorcé, ni séparé de corps',
                                'Les enfants nés et à naître' => 'Les enfants nés et à naître',
                            ];
                        @endphp
                        <input type="hidden" name="contrat_id" value="{{ $contrat->id }}">
                        <select name="beneficiaireaudeces" id="benefAuDecesSelect" class="form-select">
                            @foreach($options as $value => $label)
                                <option value="{{ $value }}" {{ $contrat->beneficiaireaudeces == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                                ...
                                <tr id="beneficiaire-row-{{ $beneficiaire->id }}">
                                    ...
                                    <td>
                                        ...
                                        <a class="deleteConfirmation" data-uuid="{{ $beneficiaire->id }}"
                                            ...
                                            data-token="{{ csrf_token() }}" data-url="{{ route('siteProposition.beneficiaires.delete', $beneficiaire->id) }}"
                                            data-title="Vous êtes sur le point de supprimer {{ $beneficiaire->nom }} {{ $beneficiaire->prenom }}"
                                            data-id="{{ $beneficiaire->id }}" data-param="0"
                                            data-route="{{ route('siteProposition.beneficiaires.delete', $beneficiaire->id) }}">
                                    ...
                                    @include('sites.pages.show.showBenef')
                                    @include('sites.pages.edit.editBenef')
        ...
        @include('sites.pages.add.addNewBenef')
        @include('sites.pages.add.addAdherentBenefModal')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                ...
                beneficiaireSelect.addEventListener("change", function () {
                    ...
                    fetch("/site/update/benef-terme/" + contratId, {
                        method: "POST",
                        headers: {
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                ...
                benefAuDecesSelect.addEventListener("change", function () {
                    ...
                    fetch("/site/update/benef-deces/" + contratId, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute("content"),
