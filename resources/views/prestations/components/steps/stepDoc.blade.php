...
<!-- JavaScript pour gérer tous les champs file -->
{{-- <script>
    let currentFileInput = null;
    let currentStream = null;
    let currentVideoElement = null;
    let currentFacingMode = 'environment'; // 'environment' pour caméra arrière, 'user' pour frontale
    let documentType = 'id-card'; // Type de document par défaut

    // Initialisation : intercepter les clics sur tous les inputs file
    document.addEventListener('DOMContentLoaded', function() {
        // Sélectionner tous les inputs de type file
        const fileInputs = document.querySelectorAll('input[type="file"]');
        
        fileInputs.forEach(input => {
            input.addEventListener('click', function(e) {
                e.preventDefault(); // Empêcher l'ouverture par défaut
                
                // Sauvegarder la référence à l'input actuel
                currentFileInput = this;
                
                // Déterminer le type de document pour adapter le guide
                ...
                if (container) {
                    ...
                    // Chercher le label ou le hidden field
                ...
                // Adapter le guide selon le type de document
                adaptGuideForDocument(docName);
                
                // Ouvrir le guide caméra
                openCameraGuide();
            });
            
            // Garder le comportement normal pour le changement de fichier
            input.addEventListener('change', function(e) {
                // Si un fichier a été sélectionné via galerie, on laisse faire
    ...
    function adaptGuideForDocument(docName) {
        ...
        // Adapter le cadre selon le type de document
    ...
    function capturePhoto() {
        ...
        // Effet flash
        document.querySelector('.guide-content').classList.add('flash-animation');
        setTimeout(() => {
            document.querySelector('.guide-content').classList.remove('flash-animation');
        }, 300);
        
        // Créer un canvas aux dimensions du cadre
        const canvas = document.createElement('canvas');
        const frame = document.getElementById('guideOverlayFrame');
        const frameRect = frame.getBoundingClientRect();
        const videoRect = video.getBoundingClientRect();
        
        // Calculer les proportions pour recadrer selon le cadre
    ...
    function uploadFromGallery() {
        ...
        // Simuler un clic sur l'input original
        if (currentFileInput) {
            // Supprimer l'attribut capture pour permettre la sélection de fichier
            const originalCapture = currentFileInput.getAttribute('capture');
            currentFileInput.removeAttribute('capture');
            
            // Ouvrir la galerie
            currentFileInput.click();
            
            // Restaurer l'attribut capture après un délai
            setTimeout(() => {
                if (originalCapture) {
                    currentFileInput.setAttribute('capture', originalCapture);
                }
            }, 1000);
        }
    }
...
{{-- <script>
        ...
        fileInputs.forEach(input => {
            input.addEventListener('click', function(e) {
                ...
                if (!isMobileDevice() || !isMediaDevicesSupported()) {
                    // Ne pas empêcher le comportement par défaut sur ordinateur
    ...
    async function startCamera() {
        ...
        try {
            ...
            // Sur ordinateur, on utilise 'user' comme facingMode par défaut
    ...
    function handleCameraError(error) {
        ...
        if (confirm('Souhaitez-vous plutôt sélectionner un fichier depuis votre ordinateur ?')) {
            hideCameraGuide();
            if (currentFileInput) {
                currentFileInput.click();
            }
    ...
    function capturePhoto() {
        ...
        const sWidth = Math.min(video.videoWidth - sx, frameRect.width * scaleX);
        const sHeight = Math.min(video.videoHeight - sy, frameRect.height * scaleY);
...
<script>
    ...
    function capturePhoto() {
        ...
        const marginPercent = 0.02; // 2% de marge
